...
# -*- coding: utf-8 -*-
"""
Created on Thursday July 10, 2025
@author: kziot

- File opens Template (templ_uTXCcyc.f3fis) with header line info, writes parametrically varying 
  parameters underneath, writes complete contents of TXC_cyclic_undrained.f3fis file
...
- a batch_undrainedTXC_cyc***.f3fis is produced populated by call commands for each file
...
- all other variables are defined inside TXC_cyclic_undrained.f3fis and can be either
...
- only the batch_undrainedTXC_cyc***.f3fis file needs to be called by FLAC3D
...
TestName = "uTXC"    # will match template and Driver and built upon that
...
sig_c    = [1,4,8]                     # initial cell pressure (atm)
alpha    = [0.0,0.1,0.2,0.3]           # static shear stress bias ratio

Test_File     = os.path.join(script_dir, "TXC_cyclic_undrained.f3fis")
Template_File = os.path.join(script_dir, "templ_uTXCcyc.f3fis")
...
for Dr_i in Dr:
    for sig_c_i in sig_c:
        for alpha_i in alpha:
                # First create a file name based on the parameters
                BaseFile = TestName + Soil+"_cyc"+"_Dr"+str(int(Dr_i*100))+"_sig"+str(sig_c_i)+"_a"+str(alpha_i)+"_Ko1"
                ...
                fileID.write("\t_sig_cell    = " + str(sig_c_i) + " \n")
...
batch_file_path = os.path.join(produce_dir, "batch_undrainedTXC_cyc.f3fis")
